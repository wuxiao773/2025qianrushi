...
#include "3rdparty/rknpu2/include/rknn_matmul_api.h"
#include "3rdparty/librga/include/im2d.hpp"
//#include "3rdparty/allocator/dma/dma_alloc.cpp"
//#include "3rdparty/allocator/drm/drm_alloc.cpp"
#include "3rdparty/rknpu2/include/Float16.h"
...
/**
 * @brief clamp：1、超过最大值，就取最大值；2、超过最小值就取最小值，否则取val
 * @param val
 * @param min
 * @param max
 * @return
...
static int v8Seg_process_i8(rknn_output *all_input, int input_id, int grid_h, int grid_w, int height, int width, int stride, int dfl_len,
                      std::vector<float> &boxes, std::vector<float> &segments, float *proto, std::vector<float> &objProbs, std::vector<int> &classId, float threshold,
                      rknn_app_context_t *app_ctx)
{
    int validCount = 0;
...
static int v8Seg_process_fp32(rknn_output *all_input, int input_id, int grid_h, int grid_w, int height, int width, int stride, int dfl_len,
                        std::vector<float> &boxes, std::vector<float> &segments, float *proto, std::vector<float> &objProbs, std::vector<int> &classId, float threshold)
{
    int validCount = 0;
    int grid_len = grid_h * grid_w;
 ...
 * @return
 */
int v8Seg_post_process(rknn_app_context_t *app_ctx, rknn_output *outputs,
                       letterbox_t *letter_box, float conf_threshold,
                       float nms_threshold, object_detect_result_list *od_results){
    std::vector<float> filterBoxes;
    std::vector<float> objProbs;
    ...
    for (int i = 0; i < 13; i++){
        ...
        if (app_ctx->is_quant){
            validCount += v8Seg_process_i8(outputs, i, grid_h, grid_w, model_in_h, model_in_w, stride, dfl_len, filterBoxes, filterSegments, proto, objProbs,
                                     classId, conf_threshold, app_ctx);
        }else{
            validCount += v8Seg_process_fp32(outputs, i, grid_h, grid_w, model_in_h, model_in_w, stride, dfl_len, filterBoxes, filterSegments, proto, objProbs,
                                       classId, conf_threshold);
        }
    }
...
int init_post_process(){
    int ret = 0;
    ret = loadLabelName(segLabelPath.c_str(), labels);
    if (ret < 0){
