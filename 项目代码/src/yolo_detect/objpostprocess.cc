...
//用obj_init_post_process方法处理存识别物品类别：v5objpostprocess.h里面定义：读取识别物品类型个数
...
//读取文件fileName识别物品类别数据，放到lines里面。max_line指定最大读取类别个数
...
//读取文件locationFilename识别物品类别数据，放到label里面
static int loadLabelName(const char *locationFilename, char *label[]){
    printf("load lable %s\n", locationFilename);
    //OBJ_CLASS_NUM在v5objpostprocess.h里面定义：读取识别物品类型个数
...
/**
 * @brief obj_post_process
 * @param app_ctx
 * @param outputs
 * @param letter_box
 * @param conf_threshold  默认的置信度阈值
 * @param nms_threshold   默认的NMS阈值
 * @param od_results
 * @return
 */
int obj_post_process(rknn_app_context_t *app_ctx, void *outputs,
    ...
    for (int i = 0; i < validCount; ++i){
        ...
        // 2. 类别索引越界保护
        if (id < 0 || id >= OBJ_CLASS_NUM) continue;
...
/**
 * @brief obj_init_post_process
 * 读取文件LABEL_NALE_TXT_PATH识别物品类别数据，放到全局label里面
 * @return
 */
int obj_init_post_process(){
    int ret = 0;
    ret = loadLabelName(objLabelPath.c_str(), labels);
    if (ret < 0){
